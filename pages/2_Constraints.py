...
try:
    ...
    subjects_cursor = db["subjects"].find({}, {"_id": 0})
    subjects = list(subjects_cursor)
except Exception:
    # Use dummy data if DB is offline for testing the UI
    subjects = [
        {"code": "CS301", "name": "Data Structures"},
        {"code": "CS304", "name": "AEC - EVS"},
        {"code": "PG101", "name": "Advanced DBMS (Core)"},
        {"code": "OE101", "name": "Open Elective - AI Base"},
    ]

# Extract subject names for selection
# We default to an empty list, ready for when the parser is done
subject_names = [s.get("name", s.get("code", "Unknown")) for s in subjects]

if not subject_names:
    st.info("No subjects found in the `subjects` collection. Once the Master Subject List parsing is complete, they will appear here.", icon="🕒")
...
default_oe = [x for x in default_oe if x in subject_names]

default_aec = current_config.get("aec", [])
default_aec = [x for x in default_aec if x in subject_names]

default_pg_core = current_config.get("pg_shared_core", "None")
if default_pg_core not in subject_names:
    default_pg_core = "None"

default_pg_pe = current_config.get("pg_shared_pe", [])
default_pg_pe = [x for x in default_pg_pe if x in subject_names]
...
with col3:
    ...
    core_options = ["None"] + subject_names
