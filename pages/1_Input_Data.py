...
# ---------------------------------------------------------------------------
tab1, tab2 = st.tabs(["👥 Faculty Allocation", "📚 Master Subject List"])

with tab1:
    st.subheader("Faculty Allocation Upload")
    ...
    if uploaded_file is not None:
        ...
        if error:
            ...
            with st.expander("🛠️ Parser Debug Info (Expand to view why it failed)"):
                st.json(debug)
            # Use return or break instead of st.stop() inside a tab context if we want other tabs to function, 
            # but standard streamlit runs top-down, so we just use an st.stop() equivalent or skip the rest.
        else:
            ...
            col1.metric("Faculty", len(records))
            col2.metric("Unique Subjects", len(all_subjects))
            col3.metric("Unique Labs", len(all_labs))

            st.divider()
            if st.button("💾 Save Faculty Data", type="primary"):
                db = get_db()
                ...
                st.success(f"✅ Saved **{len(docs)} faculty member(s)** to `{collection_name}` collection!")
            ...
            if existing:
                ...
                st.info(f"No faculty records for **{semester}** semester yet.")

with tab2:
    st.subheader("Master Subject List Upload")
    st.markdown("""
    Upload the Master Subject List Excel file.
    It should contain columns like **Subject Code**, **Subject Name**, **L**, **T**, **P** (or a merged L:T:P column), and **Semester**.
    """)
    
    subject_file = st.file_uploader(
        "Upload Subject List Excel File",
        type=["xlsx"],
        help="Upload the .xlsx file containing subject specifics.",
        key="subject_upload"
    )
    
    if subject_file is not None:
        st.info("Subject parser logic to be implemented here.")
